...
#define STX 0x02   /*PC发送到LMS的数据以STX开头 every PC->LMS packet is started by STX*/ 
#define ACKSTX 0x06 /*LMS发送到PC的数据以ACKSTX开头 every LMS->PC packet is started by ACKSTX*/
#define LEN100X1 101
#define LEN100X5 201
...
/*5种模式的命令、反馈包 the cmd and ack packets for the 5 different range/resolution modes*/
...
/*不同测量单元模式的命令、反馈包 the cmd and ack packets different measurement unit modes*/
...
//得到LMS状态的命令包 cmd packet for getting LMS status
const uchar GET_STATUS[]= {0x02,0x00,0x01,0x00,0x31,0x15,0x12};

/*设置传输率和启停测量的命令包 the cmd packets for setting transfer speed and  controlling the start and stop of measurement*/
...
/*上述命令的结束信号 the ack packet for the above commands*/
const uchar CMD_ACK[]={0x06,0x02,0x80,0x03,0x00,0xa0,0x00,0x10,0x16,0x0A};

//得到LMS型号的命令包 cmd packet for requesting LMS Type. 
...
public:
	...
	int GetSickMap( int *map );
	int GetSickMap( int *map, float scale );
